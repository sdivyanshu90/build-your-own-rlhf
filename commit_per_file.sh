...
#
# NOTES
#   * Files ignored by .gitignore (outputs/, coverage.xml, site/, *.pt, ...) are skipped.
...
desc() {
  ...
  case "$f" in
    ...
    requirements.lock)         echo "build: pin validated dependency lockset for reproducible CI" ;;
    .releaserc.json)           echo "ci: semantic-release configuration" ;;
...
for f in "${FILES[@]}"; do
  ...
  git commit -q -m "$msg" \
    -m "Done Committing $f with message: $msg"
  if [[ "$PUSH" == "true" ]]; then
